...
const pkg = {
    ...
    play: async function (id) {
        ...
        let bottom, playPause, timeElapsed, timeElapsedFront, timeElapsedMiddle, timeElapsedBack, progressBarValue, progress;
        // i put these functions in here because they relate wayy more to the play function itself than to anything else
        let createBottomBar = () => {
            bottom = new Html("div").class("flex-list").appendTo(this.wrapper).styleJs({
                position: "absolute",
                bottom: "1rem",
                zIndex: "100000000",
                width: "calc(100% - 2rem)",
    ...
    start: async function (Root) {
        ...
        let topButtons = [
            {
                text: "Home",
                action: async () => { this.home(); }
            },
            {
                text: "Settings",
                action: async () => { this.settings(); }
            },
        ]

        topButtons.forEach((b) => {
            new Html("button").text(b.text).on("click", b.action).appendTo(this.topBar);
        });
